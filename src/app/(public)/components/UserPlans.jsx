...
const UserPlans = () => {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 py-5 px-4 max-w-[85%] mx-auto">
        {plans.map((plan) => (
          <div
            key={plan.id}
            className="bg-[#0D0D0D] rounded-2xl px-8 py-10 text-white flex flex-col justify-between shadow-lg space-y-14 relative overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-xl"
            style={{ border: `1px solid ${plan.accent}20` }}
          ...
          >
              ...
              <ul className="space-y-3">
                {plan.features.map((feature, index) => (
                  <li key={index} className="flex items-center gap-2">
                    <Check className="h-5 w-5" style={{ color: plan.accent }} />
            ...
            <Link
              href="#"
              className="mt-8 w-full py-3 rounded-xl text-center font-semibold transition-all duration-300 hover:opacity-90 hover:shadow-lg z-20 relative"
              style={{ backgroundColor: plan.accent }}
            >
              {plan.btn}
            </Link>
            ...
            <div
              className={`absolute top-0 h-60 w-60 z-10 rounded-full -translate-y-1/2 transition-all duration-500 ease-out ${
                hoveredCard === plan.id
                  ? "right-0 opacity-30 scale-100"
                  : "-right-40 opacity-0 scale-75"
              }`}
              style={{ backgroundColor: plan.accent, top: "30%" }}
            ></div>
          </div>
        ))}
      </div>
